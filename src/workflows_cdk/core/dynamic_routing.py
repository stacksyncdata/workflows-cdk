...
def load_context_file(file_path: str) -> Optional[str]:
    ...
    not-found rather than a silent empty string. Read per request so edits show up
    on the next fetch; the path is derived from the module layout, never from input.
    """
...
class Router:
    ...
    def _register_core_routes(self, app: Flask) -> None:
        ...
        def app_context():
            ...
            if content is None:
                return Response(
                    data={"connector_context": None, "error": "No connector-level context.md found"},
                    status_code=404
                )
    ...
    def _register_context_route(self, route_path: str, app: Optional[Flask] = None) -> None:
        ...
        def view_func(_file=context_file, _label=route_path.lstrip("/")):
            content = load_context_file(_file)
            if content is None:
                return Response(
                    data={"module_context": None, "error": f"No context.md found for {_label}"},
                    status_code=404
                )
    ...
    def _handle_dynamic_context_request(self, dynamic_path: str) -> FlaskResponse:
        ...
        returns a clean HTTP 404 JSON body rather than a stack-trace 404.
        """
        context_file = os.path.join(os.getcwd(), self.routes_directory, dynamic_path, "context.md")
        content = load_context_file(context_file)
        if content is None:
            return Response(
                data={"module_context": None, "error": f"No context.md found for {dynamic_path}"},
                status_code=404
            )
        return Response.success(data={"module_context": content})
    ...
    def init_app(self, app: Flask) -> None:
        ...
        # Catch-all so a module without a context.md returns a clean 404
        app.add_url_rule(
