...
def load_schema_file(schema_path: str) -> dict[str, Any] | None:
...
def log_error_details(
    ...
    if not is_production_environment():
        try:
            ...
            if (
                exc_info[0] is None
            ):  # If no current exception context, create one from the error
...
def wrap_route_handler(handler: Callable) -> Callable:
    ...
    def wrapped_handler(*args: Any, **kwargs: Any) -> Any:
        try:
            ...
            required_fields = getattr(handler, "__route_info__", {}).get(
                "required_fields", []
            )
...
class Router:
    ...
    def refresh_app_config_variables(self, app_config: dict[str, Any]) -> None:
        ...
        self.local_development_settings = app_config.get(
        ...
        self.port = (
            self.local_development_settings.get("port")
            or self.app_settings.get("port")
            or 2003
        )
        self.debug = self.local_development_settings.get(
            "debug", True
        ) or self.app_settings.get("debug", True)

        # Apply Sentry and CORS settings with override priority
        self.sentry_dsn = self.app_settings.get(
            "sentry_dsn"
        ) or self.local_development_settings.get("sentry_dsn")
        self.cors_origins = self.local_development_settings.get("cors_origins") or ["*"]

        # Determine routes directory
        routes_directory_possible_key_names = [
            "routes_directory_path",
            "routes_directory",
            "routes_dir",
            "routes_path",
        ]
        self.routes_directory = next(
            (
                ...
                for key in routes_directory_possible_key_names
    ...
    def _create_route_info(
        ...
        try:
            ...
            module_dir_rel_str = str(
                relative_path.parent
            )  # Path relative to routes_directory
            ...
            if module_metadata:
                ...
                if not any(
                    m["module_id"] == module_metadata["module_id"]
        ...
        endpoint_name = (
            f"{'.'.join(path_parts)}.{function.__name__}"
            if path_parts
    ...
    def _scan_routes_directory(self) -> list:
        ...
        routes_directory = Path(
            os.path.join(current_working_directory, self.routes_directory)
        )
        ...
        for path in routes_directory.rglob("*.py"):
            ...
            if len(relative_parts) == 3 and version_dir_pattern.match(
                relative_parts[1]
            ):
    ...
    def _collect_route_information(self) -> tuple:
        ...
        for route_file_path in route_files:
            try:
                ...
                module_dir_rel_str = (
                    str(relative_path.parent.relative_to(self.routes_directory))
                    if str(relative_path.parent) != self.routes_directory
                    else ""
                )
                routes_dir_abs_str = str(routes_directory.resolve())
                ...
                if module_metadata and not any(
                    m.get("module_id") == module_metadata["module_id"]
                ...
                print_error(
                    f"Error collecting info for route file {route_file_path}: {e}"
                )
    ...
    def discover_routes(self) -> None:
        ...
        for module in module_list:
            if not any(
                m.get("module_id") == module["module_id"] for m in self.modules_list
            ):
        ...
        with clean_module_import():
            ...
            setattr(
                temporary_main_module,
                "__file__",
                os.path.join(current_working_directory, "main.py"),
            )
                ...
                for route_info in routes_info:
                    try:
                        ...
                        module_spec = importlib.util.spec_from_file_location(
                            module_name, str(route_file_path)
                        )
                        ...
                        for _, function_object in inspect.getmembers(route_module):
                            if inspect.isfunction(function_object):
                                ...
                                if hasattr(function_object, "__route_info__"):
                                    route_info = getattr(
                                        function_object, "__route_info__"
                                    )
                        ...
                        print_error(
                            f"Error while processing route file {route_file_path}: {error}"
                        )
    ...
    def _register_core_routes(self, app: Flask) -> None:
        ...
        def root():
            ...
            module_names = sorted(
                [m.get("module_name", "") for m in modules_list if m.get("module_name")]
            )
            # HTML template with Stacksync logo and connector name
            html = get_homepage_template(
                connector_name, self.app_type, self.environment, module_names
            )
            return html
        ...
        def app_config():
            try:
                ...
                return Response.success(
                    data={"app_settings": self.app_settings, "modules": modules_list}
                )
            except Exception as error:
                import traceback
        ...
        def routes():
            try:
                ...
                return Response.success(
                    data={"endpoints": serializable_routes, "route_files": routes_info}
                )
            except Exception as error:
                import traceback
    ...
    def _handle_dynamic_schema_request(self, dynamic_path: str) -> FlaskResponse:
        ...
        try:
            ...
            routes_directory = os.path.join(
                current_working_directory, self.routes_directory
            )
            schema_file_path = os.path.join(
                routes_directory, dynamic_path, "schema.json"
            )
            ...
            if not os.path.exists(schema_file_path):
                return Response(
                    data={
                        "schema": {},
                        "error": f"Schema not found for {dynamic_path}",
                    },
                    status_code=404,
                )
            ...
            try:
                ...
                if self.environment in ["dev", "development"]:
                    if not any(
                        r.get("path") == f"/{dynamic_path}/schema" for r in self.routes
                    ):
                        print(
                            f"Dynamically served schema for new path: /{dynamic_path}/schema"
                        )
                ...
                return Response(
                    data={
                        "schema": {},
                        "error": f"Invalid schema format for {dynamic_path}",
                    },
                    status_code=400,
                )
        except Exception as e:
            ...
            return Response.error(
                error=ManagedError(
                    error=e, metadata={"dynamic_path": dynamic_path}, status_code=500
                )
            )

    def _create_dynamic_schema_handler(
        self, route_path: str
        ...
        def dynamic_schema_handler() -> FlaskResponse:
            try:
                ...
                schema_file_path = os.path.join(
                    routes_directory, route_path.lstrip("/"), "schema.json"
                )
                ...
                return Response.error(
                    error=ManagedError(
                        error=e, metadata={"route_path": route_path}, status_code=500
                    )
                )

        return dynamic_schema_handler

    def _register_schema_route(self, route_path: str, app: Flask | None = None) -> None:
        ...
        if app is not None and added and hasattr(app, "add_url_rule"):
            ...
            if self.environment in ["dev", "development"]:
                print(
                    f"Registered schema route: {route_info['path']} with methods {route_info['methods']}"
                )
        ...
        for route in self.routes:
            app.add_url_rule(
                ...
                **{
                    ...
                    if k not in ["path", "endpoint", "view_func", "methods"]
                },
            )
            if self.environment in ["dev", "development"]:
                print(
                    f"Registered route: {route['path']} with methods {route['methods']}"
                )

    def route(self, rule: str | None = None, **options: Any) -> Callable:
        """
